...
int semInit(key_t key, int nsems, int* vals)
{
	...
	else if(errno == EEXIST)
	{
		...
		semid = semget(key, nsems, 0); /* get the semaphore set id */
		if(semid < 0) return -1;  /* check errno to get error message */

		/* wait for other process to initialize the semaphore */
		arg.buf = &buf;
		for(int i = 0; i < MAX_RETRIES && !ready; i++)
		{
			/* check the oparating time of the last semaphore in the set to ensure the semaphore set is initialized and ready
			 * to call semop() in this concurrent process.
			 */
			if(nsems == 0) semctl(semid, 0, IPC_STAT, arg);
			else if(nsems > 0) semctl(semid, nsems - 1, IPC_STAT, arg);
 ...
 *      If the semaphore less than or equal to 0, semP() would block
 ...
 */
int semzcnt(int semid, int semnum)
{
